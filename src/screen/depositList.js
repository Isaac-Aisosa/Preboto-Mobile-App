...
export default function DepositList ({ route, navigation }) 
{
 ...
 setToken(data)
  ...
  const FetchData = async () => {
    ...
    axios.get('http://192.168.43.240:8000/api-get-deposit-transaction/', 
    {
      headers: {
  ...
  setListItems(response.data.deposit);
  ...
  const ItemView = ({ item }) => {
    ...
    return (
      // Single Comes here which will be repeatative for the FlatListItems
        ...
        <TouchableRipple style={{width:Dimensions.get("window").width-10}} onPress={()=>navigation.navigate('DepositDetails',{
            amount: item.amount,
            time: item.timestamp,
